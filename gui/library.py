...
DEFAULT_SCRIPT_TEMPLATE = (
    ...
    "# ------------------------------\n"
    "# Diagram file/tab:\n"
    "#   open_diagram(file_path) -> name    # opens it, or switches to it if already open\n"
    "#   new_diagram() -> name              # blank diagram, no file needed (unlike open_diagram)\n"
    "#   select_diagram(name)               # switch among already-open diagrams\n"
    "#   current_diagram() -> name | None\n"
    "#   list_diagrams() -> [name, ...]\n"
    "#   save_diagram(file_path=None)       # file_path required the first time; no file picker\n"
    "# Diagram contents (act on whichever diagram is currently active):\n"
    "#   add_block(block_type, name=None, position=None) -> name\n"
    ...
    "#   find_system(block_type=None, name_contains=None) -> [name, ...]\n"
    "#   get_blocks() -> [name, ...]\n"
    "#   get_block_info(block_name)\n"
    "#   get_block_types() -> [type_name, ...]\n"
    "# SubGraphs (act on whatever level is currently active -- Top, or\n"
    "# inside a SubGraph after enter_subgraph()):\n"
    "#   enter_subgraph(block_name)   # step inside it; add_block/set_param/\n"
    "#                                # add_line etc. now build ITS contents\n"
    "#   exit_subgraph()              # step back out one level\n"
    "#   go_to_top()                  # step all the way back out\n"
    "# Simulation:\n"
    "#   sim(duration=None, dt=None, solver=None) -> {\"time\": ..., \"scopes\": {...}}\n"
    "#   get_signal(scope_name) -> (time, data)\n"
    "#   run_simulation()   # async, opens the GUI's own Run flow\n"
    "# Other:\n"
    "#   var(name)   bind a block param to a global variable `name` -- any\n"
    "#               plain top-level name this Script/the Console has defined,\n"
    "#               e.g. Kp = 2.5; set_param('G', 'Gain', var('Kp')). Same as\n"
    "#               typing 'Kp' into that param's own field.\n"
    "#   np          numpy\n"
    "#   print(text)\n"
    "# Tip: View -> Toggle Global Variables shows every variable defined so\n"
    "# far; Tools -> Clear Global Variables wipes them all for a clean slate\n"
